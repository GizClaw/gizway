...
for bundle in account gizway-user gizway-public internal-gizpay; do
...
mkdir -p "${generated_dir}/account" "${generated_dir}/gizwayuser" "${generated_dir}/gizwaypublic" "${generated_dir}/internalgizpay"
...
"${GO:-go}" run "${generator}" -generate types,client,std-http,spec -package gizwayuser -o "${generated_dir}/gizwayuser/api.gen.go" api/openapi/gizway-user.yaml
...
for package in account gizwayuser gizwaypublic internalgizpay; do
