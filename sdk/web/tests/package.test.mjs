...
test("packed SDK installs, typechecks, and bundles from root exports", () => {
  ...
  try {
    ...
    writeFileSync(join(temporary, "package.json"), JSON.stringify({ type: "module", scripts: { check: "tsc --noEmit && vite build" }, dependencies: { gizway: `file:./${filename}`, typescript: "5.9.3", vite: "8.2.2" } }));
    writeFileSync(join(temporary, "index.html"), '<script type="module" src="/src.ts"></script>\n');
    writeFileSync(join(temporary, "src.ts"), 'import { AuthenticationConfigurationError, createGizWayClient, type BrowserOAuthClient, type Region } from "gizway"; const region: Region = "global"; const oauth: BrowserOAuthClient = {clientId:"consumer",redirectUri:"https://app.example.test/callback",postLogoutRedirectUri:"https://app.example.test/"}; void createGizWayClient({entryOrigin:"https://entry.example.test", region}); void createGizWayClient({entryOrigin:"https://entry.example.test", region, oauth}); void AuthenticationConfigurationError;\n');
    ...
    const packageJSON = JSON.parse(readFileSync(join(temporary, "node_modules/gizway/package.json"), "utf8"));
    assert.equal(packageJSON.name, "gizway");
