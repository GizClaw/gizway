...
export class GizPayConnector extends APIConnector {
  protected async mapEntry(table: string, id: string, operation: string, data: Record<string, unknown>, database: CommonPowerSyncDatabase) {
    if (table === "my_merchants" && operation === "PATCH") {
      return { method: "PATCH", path: `/account/v1/merchants/${encodeURIComponent(id)}`, body: select(data, ["public_name", "status"]) };
    }
    if (table === "my_subscriptions" && operation === "PUT") {
      return { method: "POST", path: `/account/v1/products/${segment(data.product_id)}/subscriptions`, body: { id, ...select(data, ["account_id", "terms_version"]) } };
    }
    if (table === "my_subscriptions" && operation === "PATCH") {
      return { method: "PATCH", path: `/account/v1/subscriptions/${encodeURIComponent(id)}`, body: select(data, ["status"]) };
    }
    if (table === "my_subscription_keys" && operation === "PUT") {
      return { method: "POST", path: `/account/v1/subscriptions/${segment(data.subscription_id)}/keys`, body: { id, ...select(data, ["name"]) } };
    }
    if (table === "my_subscription_keys" && operation === "PATCH" && data.status === "revoked") {
      let subscriptionID = data.subscription_id;
      if (subscriptionID === undefined) {
        const rows = await database.getAll<{ subscription_id: string }>("SELECT subscription_id FROM my_subscription_keys WHERE id = ?", [id]);
        subscriptionID = rows[0]?.subscription_id;
      }
      return { method: "POST", path: `/account/v1/subscriptions/${segment(subscriptionID)}/keys/${encodeURIComponent(id)}/revoke` };
    }
    if (table === "my_topups" && operation === "PUT") {
      return { method: "POST", path: `/account/v1/accounts/${segment(data.account_id)}/topups`, body: { id, ...select(data, ["channel", "external_reference", "amount_microcredits"]) } };
    }
