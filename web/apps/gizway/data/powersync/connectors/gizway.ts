...
export class GizWayConnector extends APIConnector {
  protected mapEntry(table: string, id: string, operation: string, data: Record<string, unknown>) {
    if (table === "my_provider_keys" && operation === "PUT") {
      return { method: "POST", path: `/user/v1/providers/${segment(data.provider_id)}/keys`, body: { id, name: data.name, key: data.key, status: data.status, prices: prices(data.prices_json) } };
    }
    ...
    if (table === "my_provider_keys" && operation === "PATCH" && data.prices_json !== undefined) {
      return { method: "PUT", path: `/user/v1/provider-keys/${encodeURIComponent(id)}/prices`, body: { prices: prices(data.prices_json) } };
    }
