...
func TestServerFromArgs(t *testing.T) {
	t.Parallel()
	server, err := serverFromArgs([]string{"-address", "127.0.0.1:29300", "-credential", "risk-key"})
	if err != nil {
		t.Fatalf("serverFromArgs: %v", err)
	}
	if server.Addr != "127.0.0.1:29300" || server.ReadHeaderTimeout != 5*time.Second || server.Handler == nil {
