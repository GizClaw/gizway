...
func bootstrapAdministratorConfigFromArgs(args []string, getenv func(string) string) (bootstrapAdministratorConfig, error) {
	flags := flag.NewFlagSet("gizpay bootstrap-admin", flag.ContinueOnError)
	postgresDSN := flags.String("postgres-dsn", getenv("GIZPAY_POSTGRES_DSN"), "initialized GizPay PostgreSQL DSN")
	emailFile := flags.String("email-file", "", "file containing the initial administrator email")
	passwordFile := flags.String("password-file", "", "file containing the initial administrator password")
	displayName := flags.String("display-name", "Initial Administrator", "initial administrator display name")
...
func runBootstrapAdministrator(args []string, getenv func(string) string, output io.Writer) error {
	...
	database, err := storage.OpenGizPayPostgreSQL(config.PostgreSQLDSN, false)
	if err != nil {
		return err
	}
	defer database.Close()
	administrator, replayed, err := store.New(database.SQL).BootstrapAdministrator(
