...
type seedConfig struct {
	...
	GizPay       gizPaySeed   `yaml:"gizpay"`
	Regions      []gizWaySeed `yaml:"regions"`
}

type gizPaySeed struct {
...
func loadSeedConfig(path string, variables map[string]string) (seedConfig, []byte, error) {
	...
	if bytes.Contains(raw, []byte("${")) {
		return config, nil, errors.New("seed config contains an unresolved variable")
	}
	...
	if config.Version != 1 || strings.TrimSpace(config.AdminKeyFile) == "" || strings.TrimSpace(config.GizPay.BaseURL) == "" || len(config.Regions) == 0 {
		return config, nil, errors.New("seed config version, admin_key_file, GizPay URL, and regions are required")
	}
	...
	check := func(kind string, resources []map[string]any) error {
		for _, resource := range resources {
			...
			if previous, exists := seen[id]; exists {
				return fmt.Errorf("duplicate seed id %q in %s and %s", id, previous, kind)
			}
			seen[id] = kind
		}
		return nil
...
func seedIDs(resources []map[string]any) []string {
