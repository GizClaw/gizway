...
func TestLoadSeedConfigRejectsUnknownFieldsAndEmptyKey(t *testing.T) {
	directory := t.TempDir()
	keyPath := filepath.Join(directory, "admin-key")
	configPath := filepath.Join(directory, "seed.yaml")
	base := `version: 1
...
`
	...
	if _, _, err := loadSeedConfig(configPath, nil); err == nil || !strings.Contains(err.Error(), "empty") {
	...
	if _, _, err := loadSeedConfig(configPath, nil); err == nil || !strings.Contains(err.Error(), "unknown field") {
		t.Fatalf("unknown field error = %v", err)
	}
}

func TestLoadSeedConfigRejectsDuplicateIDsAndMissingDependencies(t *testing.T) {
	directory := t.TempDir()
	keyPath := filepath.Join(directory, "admin-key")
	if err := os.WriteFile(keyPath, []byte("admin"), 0o600); err != nil {
		t.Fatal(err)
	...
	writeAndLoad := func(contents string) error {
		path := filepath.Join(directory, "seed.yaml")
		if err := os.WriteFile(path, []byte(contents), 0o600); err != nil {
			return err
		}
		_, _, err := loadSeedConfig(path, nil)
		return err
	}
	base := `version: 1
admin_key_file: ` + keyPath + `
...
func TestLoadSeedConfigRejectsUnknownNestedPriceFields(t *testing.T) {
	directory := t.TempDir()
	keyPath := filepath.Join(directory, "admin-key")
	if err := os.WriteFile(keyPath, []byte("admin"), 0o600); err != nil {
		t.Fatal(err)
