...
func Check(openAPIDirectory, apiSourceDirectory, outputDirectory string) error {
	...
	if rootCount != 7 {
		return fmt.Errorf("expected 7 root OpenAPI documents, found %d", rootCount)
	}
	...
	for _, name := range []string{"account.yaml", "gizpay-admin.yaml", "gizpay-webhooks.yaml", "gizway-admin.yaml", "gizway-user.yaml", "gizway-public.yaml", "internal-gizpay.yaml"} {
