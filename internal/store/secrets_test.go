...
func TestSecretCipherAndDatabaseBoundaries(t *testing.T) {
	...
	payRepository, err := NewWithSecretKey(payDatabase.SQL, key)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, _, err := payRepository.CreateWebhookEndpoint(ctx, "22000000-0000-4000-8000-000000000002", "secret-test-create", payloadHash[:], webhook); err != nil {
		t.Fatal(err)
	}
	var storedWebhookSecret string
	if err := payDatabase.SQL.Get(&storedWebhookSecret, `SELECT signing_secret FROM webhook_endpoints WHERE id=$1`, webhook.ID); err != nil || !strings.HasPrefix(storedWebhookSecret, encryptedSecretPrefix) || strings.Contains(storedWebhookSecret, webhook.SigningSecret) {
		t.Fatalf("stored webhook secret = %q, %v", storedWebhookSecret, err)
	}
	if _, err := payDatabase.SQL.Exec(`INSERT INTO webhook_events(id,merchant_account_id,event_type,resource_id,payload,created_at) VALUES ('encrypted-event','22000000-0000-4000-8000-000000000002','payment_intent.succeeded','intent','{}','2026-08-10T00:00:00.000000000Z')`); err != nil {
		t.Fatal(err)
	}
	if _, err := payDatabase.SQL.Exec(`INSERT INTO webhook_deliveries(id,event_id,endpoint_id,signing_secret_snapshot,attempt,status,created_at) VALUES ('encrypted-delivery','encrypted-event','encrypted-webhook',$1,1,'pending','2026-08-10T00:00:00.000000000Z')`, storedWebhookSecret); err != nil {
		t.Fatal(err)
	}
	if err := payRepository.ClaimWebhookDelivery(ctx, "encrypted-delivery", "2026-08-10T00:00:00.000000000Z", "2026-08-10T00:00:30.000000000Z"); err != nil {
		t.Fatal(err)
	}
	rotationHash := sha256.Sum256([]byte(webhook.ID))
	if _, _, err := payRepository.RotateWebhookEndpointSecret(ctx, "22000000-0000-4000-8000-000000000002", webhook.ID, "secret-test-rotate", rotationHash[:], "whsec_rotated", "2026-08-10T00:00:01.000000000Z"); err != nil {
		t.Fatal(err)
	}
	delivery, err := payRepository.GetDeliveryTarget(ctx, "encrypted-delivery")
